        ...
        <tbody>
            @if(count($items) > 0)
                ...
                @foreach($items as $viewItem)
                    <tr id="record_id_{{$viewItem->itemId}}">
                        <td>{{$viewItem->item_name}}</td>
                        <td>
                            <a href = "/items/{{$viewItem->itemId}}" class = "btn bg-OwnInfo">View</a>
                        </td>
                        <td>
                             <a class="btn bg-OwnWarning" role="button" id="btnArchive" data_id="{{ $viewItem->itemId }}">Archive</a>
                        </td>
                    </tr>
                @endforeach
            @endif
